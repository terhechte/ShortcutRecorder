...
#define	FailWithNaiveString SRStringForKeyCode(keyCode)
    ...
    UCKeyboardLayout keyLayout;
    
    CFDataGetBytes(layoutData, CFRangeMake(0, sizeof(keyLayout)), (UInt8*)&keyLayout);
...
+ (NSBezierPath*)bezierPathWithSRCRoundRectInRect:(NSRect)aRect radius:(CGFloat)radius
{
	...
	CGFloat rad = 0.5 * MIN(NSWidth(aRect), NSHeight(aRect));
    rad = MIN(radius, rad);
	NSRect rect = NSInsetRect(aRect, rad, rad);
	[path appendBezierPathWithArcWithCenter:NSMakePoint(NSMinX(rect), NSMinY(rect)) radius:rad startAngle:180.0f endAngle:270.0f];
	[path appendBezierPathWithArcWithCenter:NSMakePoint(NSMaxX(rect), NSMinY(rect)) radius:rad startAngle:270.0f endAngle:360.0f];
	[path appendBezierPathWithArcWithCenter:NSMakePoint(NSMaxX(rect), NSMaxY(rect)) radius:rad startAngle:  0.0f endAngle: 90.0f];
	[path appendBezierPathWithArcWithCenter:NSMakePoint(NSMinX(rect), NSMaxY(rect)) radius:rad startAngle: 90.0f endAngle:180.0f];
	[path closePath];
	return path;
}
