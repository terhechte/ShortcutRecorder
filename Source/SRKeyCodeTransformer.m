...
- (id) transformedValue:(id)value
{
	...
	UniCharCount length = 4, realLength = 0;
	UniChar chars[4] = {0};
	
	CFRetain(layoutData);
    UCKeyboardLayout* keyLayout = (void*)CFDataGetBytePtr(layoutData);
    
    
	OSStatus keyTranslateError = UCKeyTranslate(keyLayout, keyCode, kUCKeyActionDisplay, 0, LMGetKbdType(), kUCKeyTranslateNoDeadKeysBit, &keysDown, length, &realLength, chars);
	CFRelease(layoutData);
	
	if (keyTranslateError != noErr) return nil;
